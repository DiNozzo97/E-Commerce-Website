...
$document = $collection->findOne(['order_number' => $orderNumber]); // Find the order
...
if (empty($document)) { // If no documents are returned (a bug has occured)
	...
	$returnArray['orderDetails']['customerName'] = $document['customer']['name'];
	...
	foreach ($document['line_items'] as $item) {
		...
		$itemDetails['barcode'] = $item['product_barcode'];
