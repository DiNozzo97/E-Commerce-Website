...
$collection = $client->movie_box->staff; // Select the database and collection      

$document = $collection->findOne(['email_address' => $testEmail]); // Find the document that contains the staff member's email address
...
if (!empty($document)) { // If a document with the email address given is returned
	if (password_verify($testPassword, $document['password_hash'])) { // If the password is correct and matches that encrypted in the hash
		$_SESSION['staffID'] = (string)$document['_id'];
